...
# ==============================================
detail = ['Simple sine forcing inhomogeneous model, same setting as GF21']
...
def All_coef():
    ...
    # ==============================================    
    global R_name
    R_name = "beta1_kappa03_Fourier_512_dt01"
    
    # ==============================================
    # Output setting
    # ==============================================
    global interval, simulationTime, outputzetas
    interval       = 10                 # sim_dt [s] Output interval
    simulationTime = 2000        # [s]
    outputzetas = False
    ...
    # Set Coef
    # ==============================================
    global Q,U, Ld_L, nu_tilt, drag_tilt, beta_tilt
    Q = 1
    nu_tilt = 1e-17              # Follow SNC
    Ld_L = 0.02
    Ld = Ld_L*L
    kappa_star  = 0.3           # given in GF21, non-dim by Ld and Q, kappa_star = kappa/sqrt(Q)
    beta_star   = 1               # given in GF21, non-dim by Ld and Q, beta_star = beta*Ld/sqrt(Q)
    
    # In this model, we non-dim by L instead of Ld
    drag_tilt = kappa_star
    beta_tilt = beta_star/Ld*L
...
# ==============================================
# Equation sets
# 2 layer Phillips model on Fourier basis
# ==============================================

problem.add_equation("dt(q1) + beta_tilt*v1 +  nu_tilt*Lap8(q1) = -J(psi1,q1) + F ",condition="(nx!=0) or (ny!=0)")
problem.add_equation(" psi1 = 0 ", condition="(nx==0) and (ny==0)")

# drag multiplied by a factor of 2, to be consistent with GF20
problem.add_equation("dt(q2) + beta_tilt*v2 + nu_tilt*Lap8(q1) + 2*drag_tilt*zeta2 = -J(psi2,q2) - F ",condition="(nx!=0) or (ny!=0)")
...
flow.add_property("0.5*(u1**2+v1**2)", name='KE')
